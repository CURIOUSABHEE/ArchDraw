  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-6 pointer-events-none">
        <div
          className="pointer-events-auto w-full max-w-xs rounded-xl shadow-2xl overflow-hidden"
        ...
        >
          ...
          <div className="p-3 space-y-0.5">
            {SHORTCUTS.map(({ key, action }) => (
              <div key={key} className="flex items-center justify-between px-2 py-1.5 rounded-md hover:bg-white/5 transition-colors">
                <span className="text-xs text-slate-400">{action}</span>
                <kbd className="text-[10px] font-mono font-medium text-slate-300 bg-white/8 border border-white/10 rounded px-1.5 py-0.5 ml-4 shrink-0">
                  {key}
                </kbd>
              </div>
            ))}
